...
export function useOpcionModificadores() {
  const { data: responseList, isLoading: isLoadingList, isError: isListError, refetch } = useOpcionModificadoresGetAllQuery();
  ...
  const formId = "opcion-modificadores-form";

  const opciones = useMemo(() => (responseList as any)?.data || [], [responseList]);
    ...
    try {
      if (editingItem) {
        const payload = { 
          ...
          precioExtra: Number(values.precioExtra)
        };
        await updateOpcion({ opcionModificadorDto: payload }).unwrap();
        addToast({ message: "Opción actualizada", variant: "success" });
      } else {
        const payload = { 
          ...values, 
          activo: values.activo === "true" || values.activo === true,
        ...
        await insertOpcion({ opcionModificadorDto: payload }).unwrap();
        addToast({ message: "Opción creada", variant: "success" });
      }
      closeModal();
      refetch();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("¿Desactivar opción?")) return;
    try {
      await deleteOpcion({ id }).unwrap();
      addToast({ message: "Opción desactivada", variant: "success" });
      refetch();
    } catch (err: any) {
  ...
  const initialValuesOverride = useMemo(() => {
    if (!editingItem) return { activo: "true", esDefault: "false", precioExtra: "0.00" };
    return {
      ...editingItem,
