...
export function useCategorias() {
  const { data: responseList, isLoading: isLoadingList, isError: isListError, refetch } = useCategoriasGetAllQuery();
  ...
  const formId = "categorias-form";

  const categorias = useMemo(() => (responseList as any)?.data || [], [responseList]);
  ...
  const handleFormikSubmit = async (values: any) => {
    ...
    try {
      if (editingItem) {
        const payload = { ...values, id: editingItem.id, activo: values.activo === "true" || values.activo === true, orden: Number(values.orden) };
        await updateCategoria({ categoriaMenuDto: payload }).unwrap();
        addToast({ message: "Categoría actualizada", variant: "success" });
      } else {
        const payload = { ...values, activo: values.activo === "true" || values.activo === true, orden: Number(values.orden) };
        await insertCategoria({ categoriaMenuDto: payload }).unwrap();
        addToast({ message: "Categoría creada", variant: "success" });
      }
      closeModal();
      refetch();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      addToast({ message: "Categoría desactivada", variant: "success" });
      refetch();
    } catch (err: any) {
